Ext.define(
	'cn.com.pingouin.vbp_client_template_modification.VbptmFloatDivisionConfigPanel'
	, {
		//
        ...
        initComponent : function() {
	        ...
	        // super apply
	        this._superclass_ = cn.com.pingouin.vbp_client_template_modification.VbptmFloatDivisionConfigPanel.superclass;
        		...
        		handler : function(button, eventObject) {
        			...
        			for(i=0; i<vbpConfigItems.length; i++) {
        				if(vbpConfigItems[i].down('combo')) {
        					// 修改取值表达式
        					if(this.idOfValueFetchExpressionConfigForEditPanel == vbpConfigItems[i].down('combo').id) {
        						this.getFloatDivision().floatDivisionSetting.valueFetchExpression = vbpConfigItems[i].down('combo').getValue();
	        					this.getFloatDivision().el.setHTML(vbpConfigItems[i].down('combo').getValue());
        ...
        beforeApplyConigurations : function(configurations) {
      	...
      	getValueFetchExpressionConfigComboStore : function() {
      		var valueFetchExpressions = this.getValueFetchExpressions();
      		
      		var comboData = new Array();
      		
      		for(i=0; i<valueFetchExpressions.length; i++) {
      			if(valueFetchExpressions[i].value) {
      				if(valueFetchExpressions[i].name && valueFetchExpressions[i].name.length > 0) {
      					comboData.push(
      							{
      								"comboItemValue" : valueFetchExpressions[i].value, 
      								"comboItemName" : valueFetchExpressions[i].name
      							}
      					);
      				} else {
      					comboData.push(
      							{
      								"comboItemValue" : valueFetchExpressions[i].value, 
      								"comboItemName" : valueFetchExpressions[i].value
      							}
      					);
      				}
      			}
      	...
      	getValueFetchExpressionConfigPanel : function() {
      		...
      		return {
	            ...
	            , {
	            	xtype : 'combo',
                  	flex : 1,
                  	store : this.getValueFetchExpressionConfigComboStore(),
