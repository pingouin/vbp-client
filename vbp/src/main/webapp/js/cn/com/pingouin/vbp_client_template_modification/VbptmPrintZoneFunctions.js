Ext.define(//
    'cn.com.pingouin.vbp_client.VbptmPrintZoneFunctions'//
    , {
    	initComponent : function() {
    		cn.com.pingouin.vbp_client_template_modification.VbptmPrintZoneFunctions.superclass.initComponent.apply(this, arguments);
    	},
...
//			LODOP.PREVIEW();
		...
		openPrintZoneConfigWindow : function() {
			var vbptmBasicSettingConfigWindow = Ext.create(
            ...
            vbptmBasicSettingConfigWindow.show();
		},
		
		printZonePreviewWithoutData : function(LODOP) {
			var vbptmPrintZonePanel = Ext.getCmp(cn.com.pingouin.vbp_client.utils.ConstantsForComponentId.id_printZonePanel);

			var vbptmConfiguration = new Object();
			vbptmConfiguration['basicSetting'] = vbptmPrintZonePanel.vbpConfiguration.basicSetting;
			vbptmConfiguration['floatDivisionSettings'] = vbptmPrintZonePanel.vbpConfiguration.floatDivisionSettings;
			
			var leftOffset = 0;
    		var topOffset = 0;
    		if(Number(vbptmConfiguration.basicSetting.printOffsetX)) {
    			leftOffset = Number(vbptmConfiguration.basicSetting.printOffsetX);
    		}
    		if(Number(vbptmConfiguration.basicSetting.printOffsetY)) {
    			topOffset = Number(vbptmConfiguration.basicSetting.printOffsetY);
    		}
    		
			var printOrientation = 2;
			if(vbptmConfiguration.basicSetting.printOrientation == 'portrait') {
				printOrientation = 1;
			}
	    	...
	    	for(i=0; i<floatDivisionItems.length; i++) {
	    		if(cn.com.pingouin.vbp_client.utils.ConstantsForFloatType.dynamicTextFloatDivisionType == floatDivisionItems[i].floatDivisionType) {
	    			var floatDivisionTop = floatDivisionItems[i].el.getOffsetsTo(vbptmPrintZonePanel.el)[1] * pixelRelateCm + topOffset;
	    			var floatDivisionLeft = floatDivisionItems[i].el.getOffsetsTo(vbptmPrintZonePanel.el)[0] * pixelRelateCm + leftOffset;
	    			var floatDivisionWidth = floatDivisionItems[i].getWidth() * pixelRelateCm;
	    			var floatDivisionHeight = floatDivisionItems[i].getHeight() * pixelRelateCm;
	    			...
	    			LODOP.ADD_PRINT_HTM(
	    					...
	    					floatDivisionWidth + "cm", 
	    					floatDivisionHeight + "cm", 
	    					"<span " + " style=" + floatDivisionStyle + ">" + floatDivisionItems[i].floatDivisionSetting.valueFetchExpression + "</span>"
	    			);
	    		} else if(cn.com.pingouin.vbp_client.utils.ConstantsForFloatType.pictureFloatDivisionType == floatDivisionItems[i].floatDivisionType) {
	    			var floatDivisionTop = floatDivisionItems[i].el.getOffsetsTo(vbptmPrintZonePanel.el)[1] * pixelRelateCm + topOffset;
	    			var floatDivisionLeft = floatDivisionItems[i].el.getOffsetsTo(vbptmPrintZonePanel.el)[0] * pixelRelateCm + leftOffset;
	    			...
	    			LODOP.ADD_PRINT_IMAGE(
	    					...
	    					floatDivisionWidth + "cm", 
	    					floatDivisionHeight + "cm", 
	    					"<span>" + floatDivisionItems[i].floatDivisionSetting.valueFetchExpression + "</span>"
	    			);
	    		} else if(cn.com.pingouin.vbp_client.utils.ConstantsForFloatType.staticTextFloatDivisionType == floatDivisionItems[i].floatDivisionType) {
	    			var floatDivisionTop = floatDivisionItems[i].el.getOffsetsTo(vbptmPrintZonePanel.el)[1] * pixelRelateCm + topOffset;
		   			var floatDivisionLeft = floatDivisionItems[i].el.getOffsetsTo(vbptmPrintZonePanel.el)[0] * pixelRelateCm + leftOffset;
		...
		openTemplatePropertyConfigWindow : function () {
			var vbptmTemplatePropertyConfigWindow = Ext.create(
			...
			vbptmTemplatePropertyConfigWindow.show();
		},
		...
		vbpTemplateSave : function() {
			...
			var vbpConfiguration = new Object();
			vbpConfiguration['basicSetting'] = vbptmPrintZonePanel.vbpConfiguration.basicSetting;
			vbpConfiguration['floatDivisionSettings'] = floatDivisionSettings;
			if(vbptmPrintZonePanel.vbpConfiguration.templatePropertySetting) {
				vbpConfiguration['templatePropertySetting'] = vbptmPrintZonePanel.vbpConfiguration.templatePropertySetting;
			}
			
			var templateSaveCallBackUrl = vbptmPrintZonePanel.vbpConfiguration.basicSetting.templateSaveCallBackUrl;
			
			var ajaxParameters = new Object();
			ajaxParameters['vbpConfiguration'] = Ext.encode(vbpConfiguration);
		...
		vbpTemplateSaveAs : function() {
			...
			var templateSaveAsCallBackUrl = vbptmPrintZonePanel.vbpConfiguration.basicSetting.templateSaveAsCallBackUrl;
			
			var ajaxParameters = new Object();
			ajaxParameters['vbpConfiguration'] = Ext.encode(vbpConfiguration);
			
			if(templateSaveAsCallBackUrl && templateSaveAsCallBackUrl.length > 0) {
				Ext.data.JsonP.request({
					url : templateSaveAsCallBackUrl,
					timeout : cn.com.pingouin.vbp_client.utils.ConstantsForSystemConfig.defaultTimeoutMillisecondsForAjax,
					params : ajaxParameters,
					success : function(response, options) {
						// var templateSaveAsResponseObject = Ext.decode(response.responseText, true);
